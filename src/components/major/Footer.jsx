  ...
  productLinks: [
    {
      ...
      url: "/product-category/mobila-bransletankar",
    },
    {
      label: "Station\u00e4ra br\u00e4nsletankar",
      url: "/product-category/stationara-bransletankar",
    },
  ],
  quickLinks: [
...
function resolveFooterData(optionsRoot, language = DEFAULT_LANGUAGE) {
  ...
  const socialLinks = Array.isArray(socialSource)
    ? socialSource
        .map((item) => {
          ...
          const uploadedIcon =
            item.icon?.url || item.logo?.url || item.icon || item.logo;
  ...
  return {
    cta: {
      ...
      buttonText:
        cta.cta_button_text || cta.button_text || DEFAULT_FOOTER.cta.buttonText,
    ...
    quickLinks: normalizeList(
      footer?.quick_links ||
        global?.quick_links ||
        global?.quick_links_group?.quick_links,
      DEFAULT_FOOTER.quickLinks,
      language,
    ),
    supportLinks: normalizeList(
      footer?.support_links ||
        global?.support_links ||
        global?.support_links_group?.support_links ||
        global?.resources?.resource_links,
  ...
  return (
    <footer className="bg-[#18201F] text-white">
      <div className="web-width px-6 md:px-0 pb-14 pt-14">
        <div className="relative mb-12 overflow-hidden rounded-lg bg-[#303736] px-8 py-10 md:px-12 md:py-12">
          ...
          {footer.cta.buttonText && (
            <Link
              ...
              className="group relative z-10 mt-8 inline-flex h-13 items-center gap-4 rounded-sm bg-[var(--color-yellow)] py-1.5 pr-1.5 pl-6 font-heading text-[14px] tracking-[-0.28px] text-black transition-opacity hover:opacity-90 md:absolute md:right-12 md:top-1/2 md:mt-0 md:-translate-y-1/2"
            >
          ...
          <div
            className="pointer-events-none absolute right-0 top-0 h-full w-105 opacity-30"
            style={{
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-[2.1fr_1fr_1fr_1.35fr_0.85fr] lg:gap-20">
          <div>
                ...
                >
                  ...
                  >
                    ...
                    <Image
                      ...
                      width={15}
                      height={15}
                      className="h-3.75 w-3.75 transition-transform"
                    />
                  </Link>
                </li>
              ))}
            ...
            <Image
              ...
              width={193}
              height={65}
              className="mt-7 h-auto w-[150px] md:w-[193px]"
            />
          </div>
          ...
          <div>
            ...
            <div className="mt-5 space-y-4 font-body text-[14px] leading-5.5 tracking-[-0.28px] text-white">
              <Link
                ...
                >
                  {item.icon && (
                    <Image
                      ...
                      width={14}
                      height={14}
                      className="max-h-3.5 w-3.5 object-contain"
                    />
                  )}
                </Link>
              ))}
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-2">
            {footer.legalLinks.map((item, index) => (
              ...
              >
                {index > 0 && (
                  <span
                    className="h-2.25 w-px bg-white/60"
                    aria-hidden="true"
                  />
                )}
                <Link
        ...
        {links.map((item) => (
          <li key={`${item.label}-${item.url}`}>
            <Link
              href={item.url}
              target={item.target}
              className="font-body text-[14px] leading-5.5 tracking-[-0.28px] text-white transition-opacity hover:opacity-70"
            >
              {item.label}
            </Link>
          </li>
